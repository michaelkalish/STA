...
/**
 * Utility to create
 * <a href="http://en.wikipedia.org/wiki/Combination"> combinations</a>
 * {@code (n, k)} of {@code k} elements in a set of {@code n} elements.
 * 
 * 
 * TODO: reimplement this - it creates new int arrays all the time
 *
 * @since 3.3
 */
public class Combinations implements Iterable<int[]> {
	...
	public Iterator<int[]> iterator() {
		if (k == 0 || k == n) {
			return new SingletonIterator(natural(k));
		}

		return new LexicographicIterator(n, k);
	}

	private int[] natural(int k2) {
		int[] ret = new int[k2];
		for (int i = 0; i < k2; i++)
			ret[i] = i;
		return ret;
	...
	private static class LexicographicComparator implements Comparator<int[]>, Serializable {
		...
		public int compare(int[] c1, int[] c2) {
			...
			final int[] c1s = copyOf(c1);
			Arrays.sort(c1s);
			final int[] c2s = copyOf(c2);
			Arrays.sort(c2s);

...
public class CholeskyFactorization {
	 ...
	 * @param rescaler
	 *            a matrix rescaling implementation for balancing the
	 *            factorization (it can be null in order to avoid rescaling)
	 * @throws Exception
	 */
	...
	public void factorize(boolean checkSymmetry) throws Exception {
		...
		if (this.rescaler != null) {
			...
			// and because U is diagonal it preserves the triangular form of
			// U.L, so
	...
	public DoubleMatrix2D getL() {
		if (this.L == null) {
			...
			if (this.rescaler != null) {
				// Q = UInv.Q1.UInv[T] = UInv.L1.L1[T].UInv[T] =
				// (UInv.L1).(UInv.L1)[T]
				// so
				// L = UInv.L1

 ...
 * where x, q &#8712; R<sup>n</sup>, P is a symmetric nXn matrix and r &#8712;
 * R.
 * 
 * <br>
 ...
 * f(x) = x.P.x for a symmetric matrix P (f can even be written as x.P1.x with
 * P1 not symmetric, for example <br>
 * f = x^2 + 2 x y + y^2 we can written with P={{1, 1}, {1, 1}} symmetric or
 * with P1={{1, -1}, {3, 1}} not symmetric, but here we are interested in
 * symmetric matrices for they convexity properties).</li>
 * <li>Let f(x) = x.P.x be a quadratic form with associated symmetric matrix P,
 * then we have:
 * <ul>
 ...
 * </ol>
 * 
 * NOTE2: precisely speaking, this class should have been named
 * "PolynomialOfDegreeTwo", because by definition a quadratic form in the
 * variables x1,x2,...,xn is a polynomial function where all terms in the
 * functional expression have order two. A general polynomial function f(x) of
 * degree two can be written as the sum of a quadratic form Q = x.P.x and a
 * linear form L = q.x (plus a constant term r): <br>
 * f(x) = Q + L + r <br>
